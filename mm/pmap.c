...
void mips_detect_memory()
{
    ...
    basemem = 0x4000000; /* 2^26 64MB */

	// Step 2: Calculate corresponding npage value.
    npage = basemem >> PGSHIFT;   /* 2^14 */
...
void boot_map_segment(Pde *pgdir, u_long va, u_long size, u_long pa, int perm)
{
    ...
    // if (size % BY2PG != 0 ) {
    //     panic("In function boot_map_segment: size is not a multiple of BY2PG.");
    //     return;
    // }
...
void page_init(void)
{
    ...
    for (; page2ppn(p) < npage; p++) {
        /* do not insert the page that trap will use*/
        if (page2kva(p) == TIMESTACK - BY2PG) {
            continue;
        }
        p->pp_ref = 0;
