 ...
 * va: the virtual address that the segment need to be load at
 * sgsize: The size of the segment in memory
 * bin: begin address in ELF file
 * bin_size: The size of the segment in file
 */
/*** exercise 3.6 ***/
...
load_icode(struct Env *e, u_char *binary, u_int size)
{
    ...
    /* Step 1: alloc a page. */
    if ((r = page_alloc(&p)) < 0) {
        // panic("load_icode : step1:You are failure");
        return;
    }
    ...
    if ((r = page_insert(e->env_pgdir, p, USTACKTOP - BY2PG, perm)) < 0) {
        // panic("load_icode : step2:You are failure");
        return;
    }
    ...
    if (r < 0) {
        // panic("load_icode : step3:you are failure");
        return;
    }
