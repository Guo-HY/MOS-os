...
#define 	OUTPUT(arg, s, l)  \
  { if (((l) < 0) || ((l) > LP_MAX_BUF)) { \
    ...
    */
    // format type: %[flags][width][.precision][length]specifier
	 ...
	 int length, int ladjust, char padc, int upcase)
{
    /* 
     *  param:
     *      输出数组，输入数字，基数，负数标志，打印数字的宽度，左对齐标志，填充字符，是否大写
     *  algorithm :
	...
	*p++ = '-';
    ...
    if (negFlag && !ladjust && (padc == '0')) { //如果是负数并且右对齐并且填充0
