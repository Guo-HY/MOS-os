...
int sys_env_alloc(void)
{
    ...
    struct Trapframe *tmp = (struct Trapframe*)(KERNEL_SP - sizeof(struct Trapframe));
    bcopy((void*)tmp, (void*)(&e->env_tf), sizeof(struct Trapframe));
    e->env_status = ENV_NOT_RUNNABLE;
    e->env_tf.regs[2] = 0;  // set child return value to zero
...
{
        // Your code here
    u_int kseg1 = 0xa0000000;
    u_int dev_start_addr[] = {0x10000000, 0x13000000, 0x15000000};
    u_int dev_length[] = {0x20, 0x4200, 0x200};
    int i;
    if (va > ULIM) { return -E_INVAL; }
    for (i = 0; i < 3; i++) {
        if (dev_start_addr[i] <= dev && dev + len - 1 < dev_start_addr[i] + dev_length[i]) { break; }
    }
    if (i == 3) { return -E_INVAL; }
    bcopy((void*)va, (void*)(dev + kseg1), len);
    return 0;
}
