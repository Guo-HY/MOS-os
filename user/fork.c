...
pgfault(u_int va)
{
    ...
    if (!(perm & PTE_COW)) {
        user_panic("User pgfault deal a not COW page!");
    }
	//map the new page at a temporary place
    va = ROUNDDOWN(va, BY2PG);
    tmp = (u_int *)USTACKTOP;
    if ((ret = syscall_mem_alloc(0, tmp, PTE_V | PTE_R)) < 0) {
        user_panic("User pgfault syscall_mem_alloc faild!");
    }
	//copy the content
    user_bcopy((void *)va, (void *)tmp, BY2PG);
	//map the page on the appropriate place
    if ((ret = syscall_mem_map(0, tmp, 0, va, PTE_V | PTE_R)) < 0) {
        user_panic("User pgfault syscall_mem_map faild!");
    }
	//unmap the temporary place
    if ((ret = syscall_mem_unmap(0, tmp)) < 0) {
        user_panic("User pgfault syscall_mem_unmap faild!");
    }
    return;
}
/* Overview:
 ...
 * our mapping should be clopy-on-write if the page is writable or
 * copy-on-write.
 *
 * Hint:
...
fork(void)
{
    ...
    if (newenvid == 0) {
        ...
        envid = ENVX(envid);
        env = &envs[envid]; // envs is defined in entry.S
        return 0;
    } else {
        // parent process
        ...
        if (r < 0) {
            user_panic("fork : syscall_mem_alloc error");
        }
        r = syscall_set_pgfault_handler(newenvid, __asm_pgfault_handler, UXSTACKTOP);
        if (r < 0) {
            user_panic("fork : syscall_set_pgfault_handler error");
        }

        r = syscall_set_env_status(newenvid, ENV_RUNNABLE);
        if (r < 0) {
            user_panic("fork : syscall_set_env_status error");
        }
    }
